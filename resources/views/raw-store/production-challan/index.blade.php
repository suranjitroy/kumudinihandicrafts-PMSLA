        ...
        <div class="row mb-2">
          ...
          <div class="col-sm-6">
            <ol class="breadcrumb float-sm-right">
              ...
              <li class="breadcrumb-item active">Production Challan</li>
            </ol>
          </div><!-- /.col -->
        </div><!-- /.row -->
      </div><!-- /.container-fluid -->
          ...
          <div class="col-md-12">
            ...
            <div class="card">
              <div class="card-header">
                <h3 class="card-title">Production Challan List</h3>
              </div>
              <!-- /.card-header -->
              <div class="card-body">
                
                  <a href="{{ route('production-challan.create') }}" class="btn btn-primary"><i class="fa fa-plus"></i>Add New Challan</a>    
                ...
                <table id="store" class="table table-bordered table-striped">
                  ...
                  <tbody>
                    ...
                    <tr>
                      <td>{{ $key + 1 }}</td>
                      <td>{{ $data->pro_challan_date }}</td>
                      <td>{{ $data->pro_challan_no }}</td>
                      <td>{{ $data->workOrder->production_order_no }}</td>
                      <td>{{ $data->item->name}}</td>
                      <td>{{ $data->materialSetup->material_name }}</td>
                      <td>{{ $data->total_quantity }}</td>
                      <td>{{ $data->assign_quantity_total }}</td>
                     <td>
                       @if($data->status == 0)
                      ...
                      <td>
                         @role('Store Staff')
                          <a href="{{ route('production-challan.show', $data->id) }}" class="btn btn-info"><i class="fa fa-eye"></i>
                          </a>
                          @if($data->status == !2)
                          <a href="{{ route('production-challan.edit', $data->id) }}" class="btn btn-info"><i class="fa fa-edit"></i>
                          </a>
                          <form action="{{ route('production-challan.destroy', $data->id) }}" method="POST" class="d-inline">
                          @csrf
                          @method('DELETE')
