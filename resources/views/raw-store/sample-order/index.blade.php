          ...
          <div class="col-md-12">
            ...
            <div class="card">
              <div class="card-header">
                <h3 class="card-title">Sample Order List</h3>
              </div>
              <!-- /.card-header -->
              <div class="card-body">
                
                  <a href="{{ route('sample-work-order.create') }}" class="btn btn-primary"><i class="fa fa-plus"></i> Add Sample Order</a>
                ...
                <table id="store" class="table table-bordered table-striped">
                  ...
                  <tbody>
                    ...
                    <tr>
                      <td>{{ $key + 1 }}</td>
                      <td>{{ $data->sample_order_no }}</td>
                      <td>{{ $data->order_entry_date}}</td>
                      <td>{{ $data->masterInfo->name }}</td>
                      <td>{{ $data->item->name }}</td>
                      <td>
                          @foreach($data->sampleWorkOrderFabricItem->unique('material_setup_id') as $fabric)
                     ...
                     <td>
                      ...
                      <td>
                         @role('Store Staff')
                          <a href="{{ route('sample-work-order.show', $data->id) }}" class="btn btn-info"><i class="fa fa-eye"></i>
                          </a>
                          @if($data->status == !2)
                          <a href="{{ route('sample-work-order.edit', $data->id) }}" class="btn btn-info"><i class="fa fa-edit"></i>
                          </a>
                          <form action="{{ route('sample-work-order.destroy', $data->id) }}" method="POST" class="d-inline">
                          @csrf
                          @method('DELETE')
