        ...
        <div class="row mb-2">
          ...
          <div class="col-sm-6">
            <ol class="breadcrumb float-sm-right">
              ...
              <li class="breadcrumb-item active">Edit Embroidery Order Receive</li>
            </ol>
          </div><!-- /.col -->
        </div><!-- /.row -->
      </div><!-- /.container-fluid -->
          ...
          <div class="col-md-6">
            ...
            <div class="card card-primary">
              <div class="card-header">
                <h3 class="card-title">Embroidery Order Receive Edit</h3>
              </div>
              <!-- /.card-header -->
                <form method="POST" action="{{ route('embroidery-receive.update', $receive->id) }}">
                   @method('PUT')
                @csrf
                <div class="card-body">
                  <div class="form-group">
                    ...
                    <input type="text" class="form-control" placeholder="receive date" id="receive_date" name="receive_date" value="{{ $receive->receive_date }}">
                  </div>
                  <div class="form-group">
                    @error('store_id')
                    <div class="alert alert-danger">{{ $message }}</div>
                    ...
                    @foreach ( $embOrders as $embOrder)
                      <option value="{{ $embOrder->id }}" @if($embOrder->id == $receive->embroidery_order_id) selected="selected"  @endif>{{ $embOrder->emb_order_no }}</option>
                    @endforeach
                    </select>
                  </div>
                  <div class="form-group">
                    ...
                    <input type="text" class="form-control" placeholder="quantity" name="receive_quantity" value="{{ $receive->receive_quantity }}">
                  </div>
                  <div class="form-group">
                    @error('phn_no')
                         <div class="alert alert-danger">{{ $message }}</div>
