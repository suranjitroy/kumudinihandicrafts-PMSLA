        ...
        <div class="row mb-2">
          ...
          <div class="col-sm-6">
            <ol class="breadcrumb float-sm-right">
              ...
              <li class="breadcrumb-item active">Edit Embroidery Order Process</li>
            </ol>
          </div><!-- /.col -->
        </div><!-- /.row -->
      </div><!-- /.container-fluid -->
          ...
          <div class="col-md-6">
            ...
            <div class="card card-primary">
              <div class="card-header">
                <h3 class="card-title">Embroidery Order Process Edit</h3>
              </div>
              <!-- /.card-header -->
                <form method="POST" action="{{ route('embroidery-process.update', $processing->id) }}">
                   @method('PUT')
                @csrf
                <div class="card-body">
                  <div class="form-group">
                    ...
                    <input type="text" class="form-control" placeholder="entry date" id="entry_date" name="entry_date" value="{{ $processing->entry_date }}">
                  </div>
                  <div class="form-group">
                    @error('store_id')
                    <div class="alert alert-danger">{{ $message }}</div>
                    ...
                    @foreach ( $embOrders as $embOrder)
                      <option value="{{ $embOrder->embroidery_order_id }}" @if($embOrder->embroidery_order_id == $processing->embroidery_order_id) selected="selected"  @endif>{{ $embOrder->embroideryOrder->emb_order_no }}</option>
                    @endforeach
                    </select>
                  </div>
                  <div class="form-group">
                    ...
                    <div class="alert alert-danger">{{ $message }}</div>
                    @enderror
                    <label>Process Section</label>
                    <select class="form-control select2" name="process_section_id" id="process_section_id">
                      <option>Select preocess section</option>
                    @foreach ( $processSections as $processSection)
                      <option value="{{ $processSection->id }}"  @if($processSection->id == $processing->process_section_id) selected="selected"  @endif>{{ $processSection->name }}</option>
                    @endforeach
                    </select>
                  </div>
                  ...
                  <div class="form-group">
                    @error('phn_no')
                         <div class="alert alert-danger">{{ $message }}</div>
                    @enderror
                    <label for="exampleInputEmail1">Dispatch Quantity</label>
                    <input type="text" class="form-control" placeholder="quantity" name="dispatch_quantity" value="{{ $processing->dispatch_quantity }}">
                  </div>
                  <div class="form-group">
                    @error('phn_no')
                         <div class="alert alert-danger">{{ $message }}</div>
