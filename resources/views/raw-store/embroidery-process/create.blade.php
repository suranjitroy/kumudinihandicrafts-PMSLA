        ...
        <div class="row mb-2">
          ...
          <div class="col-sm-6">
            <ol class="breadcrumb float-sm-right">
              ...
              <li class="breadcrumb-item active">Add Embroidery Order Process</li>
            </ol>
          </div><!-- /.col -->
        </div><!-- /.row -->
      </div><!-- /.container-fluid -->
          ...
          <div class="col-md-6">
            ...
            <div class="card card-primary">
              <div class="card-header">
                <h3 class="card-title">Embroidery Order Process Entry</h3>
              </div>
              <!-- /.card-header -->
                <form method="POST" action="{{ route('embroidery-process.store') }}">
                @csrf
                <div class="card-body">
                  <div class="form-group">
                    ...
                    <input type="text" class="form-control" placeholder="entry date" id="entry_date" name="entry_date">
                  </div>
                  <div class="form-group">
                    @error('store_id')
                    <div class="alert alert-danger">{{ $message }}</div>
                    ...
                    @foreach ( $embOrders as $embOrder)
                      <option value="{{ $embOrder->embroidery_order_id }}">{{ $embOrder->embroideryOrder->emb_order_no }}</option>
                    @endforeach
                    </select>
                  </div>
                  <div class="form-group">
                    ...
                    @foreach ( $processSections as $processSection)
                      <option value="{{ $processSection->id }}">{{ $processSection->name }}</option>
                    @endforeach
                    </select>
                  </div>
                  ...
                  <div class="form-group">
                    ...
                    <input type="text" class="form-control" placeholder="quantity" name="dispatch_quantity">
                  </div>
                  <div class="form-group">
                    @error('phn_no')
                         <div class="alert alert-danger">{{ $message }}</div>
